	...
	rewriteBetaToV1 := func(w http.ResponseWriter, r *http.Request) {
		...
		if isOperation {
			...
			if len(captured.body) > 0 {
				...
				bodyStr = strings.ReplaceAll(bodyStr, "global/healthChecks/computehttphealthcheck-", "global/httpHealthChecks/computehttphealthcheck-")
				bodyStr = strings.ReplaceAll(bodyStr, "global/healthChecks/computehttpshealthcheck-", "global/httpsHealthChecks/computehttpshealthcheck-")
				bodyBytes := []byte(bodyStr)
